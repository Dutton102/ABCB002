...
foreach ($cont_posts as $key => $item) {
  ...
  /*content*/
  if(isset($params[0]['port_social_icons'])  && in_array(0,$params[0]['port_social_icons'])) {
    $cont['social_facebook']= get_post_meta($item->ID, 'social_facebook', TRUE);
  }else{
    $cont['social_facebook']="";
  }
  if(isset($params[0]['port_social_icons'])  && in_array(1,$params[0]['port_social_icons'])) {
    $cont['social_instagram']= get_post_meta($item->ID, 'social_instagram', TRUE);
  }else{
    $cont['social_instagram']="";
  }
  if(isset($params[0]['port_social_icons'])  && in_array(2,$params[0]['port_social_icons'])) {
    $cont['social_twitter']= get_post_meta($item->ID, 'social_twitter', TRUE);
  }else{
    $cont['social_twitter']="";
  }
  if(isset($params[0]['port_social_icons'])  && in_array(3,$params[0]['port_social_icons'])) {
    $cont['social_google']= get_post_meta($item->ID, 'social_google', TRUE);
  }else{
    $cont['social_google'] = "";
  }
  /*popup*/
  if(isset($params[0]["port_popup_social_icons"])  && in_array(0,$params[0]["port_popup_social_icons"])) {
    $cont['popup_social_facebook']= get_post_meta($item->ID, 'social_facebook', TRUE);
  }else{
    $cont['popup_social_facebook']="";
  }
  if(isset($params[0]["port_popup_social_icons"])  && in_array(1,$params[0]["port_popup_social_icons"])) {
    $cont['popup_social_instagram']= get_post_meta($item->ID, 'social_instagram', TRUE);
  }else{
    $cont['popup_social_instagram']="";
  }
  if(isset($params[0]["port_popup_social_icons"])  && in_array(2,$params[0]["port_popup_social_icons"])) {
    $cont['popup_social_twitter']= get_post_meta($item->ID, 'social_twitter', TRUE);
  }else{
    $cont['popup_social_twitter']="";
  }
  if(isset($params[0]["port_popup_social_icons"])  && in_array(3,$params[0]["port_popup_social_icons"])) {
...
$showCase3_conts = json_encode($conts);

/*- Contact's count -*/
$viewCount  = isset($params[0]['port_cont_count_in_page'])?$params[0]['port_cont_count_in_page']:'8';
/*- Upload 'No Image' -*/
$noImage  = isset($params[0]['port_no_image'])?$params[0]['port_no_image']:''; 
...
<div class="<?php echo $theme; ?> sc_container" id="theme">
    <!-- SEARCH -->
	<div id="Portfolio_search" class="search_<?php echo $staff_uniqid;?>">
	...
	<script type="text/javascript">
		var showCase3_cont = new contactView(<?php echo $showCase3_conts;?>,<?php echo $viewCount;?>,'<?php echo $theme;?>','Portfolio',<?php echo $search_type;?>,<?php echo get_option('lightbox')?>, '<?php echo get_option('enable_message'); ?>', '<?php echo $noImage; ?>' , '<?php echo $staff_uniqid;?>');
    </script>
</div>
