...
foreach ($cont_posts as $key => $item) {
  ...
  /*content*/
  if(isset($params[0]['blog_social_icons'])  && in_array(0,$params[0]['blog_social_icons'])) {
    $cont['social_facebook']= get_post_meta($item->ID, 'social_facebook', TRUE);
  }else{
    $cont['social_facebook']="";
  }
  if(isset($params[0]['blog_social_icons'])  && in_array(1,$params[0]['blog_social_icons'])) {
    $cont['social_instagram']= get_post_meta($item->ID, 'social_instagram', TRUE);
  }else{
    $cont['social_instagram']="";
  }
  if(isset($params[0]['blog_social_icons'])  && in_array(2,$params[0]['blog_social_icons'])) {
    $cont['social_twitter']= get_post_meta($item->ID, 'social_twitter', TRUE);
  }else{
    $cont['social_twitter']="";
  }
  if(isset($params[0]['blog_social_icons'])  && in_array(3,$params[0]['blog_social_icons'])) {
  ...
  /*popup*/
  if(isset($params[0]['blog_popup_social_icons'])  && in_array(0,$params[0]['blog_popup_social_icons'])) {
    $cont['popup_social_facebook']= get_post_meta($item->ID, 'social_facebook', TRUE);
  }else{
    $cont['popup_social_facebook']="";
  }
  if(isset($params[0]['blog_popup_social_icons'])  && in_array(1,$params[0]['blog_popup_social_icons'])) {
    $cont['popup_social_instagram']= get_post_meta($item->ID, 'social_instagram', TRUE);
  }else{
    $cont['popup_social_instagram']="";
  }
  if(isset($params[0]['blog_popup_social_icons'])  && in_array(2,$params[0]['blog_popup_social_icons'])) {
    $cont['popup_social_twitter']= get_post_meta($item->ID, 'social_twitter', TRUE);
  }else{
    $cont['popup_social_twitter']="";
  }
  if(isset($params[0]['blog_popup_social_icons'])  && in_array(3,$params[0]['blog_popup_social_icons'])) {
...
$Blog_conts = json_encode($conts);
/*- Contact's count -*/
$viewCount  = isset($params[0]['blog_cont_count_in_page'])?$params[0]['blog_cont_count_in_page']:'6';
/*- Upload 'No Image' -*/
$noImage  = isset($params[0]['blog_no_image'])?$params[0]['blog_no_image']:''; 
...
<div class="<?php echo $theme; ?> sc_container" id="theme">
    <!-- Search -->
	<div id="blog_search" class="search_<?php echo $staff_uniqid;?>">
	...
	<!-- Contacts -->	
	<div id="blog_contact" class="parentDiv">	
    ...
    <script type="text/javascript">
      var showCase4_cont = new contactView(<?php echo $Blog_conts;?>,<?php echo $viewCount;?>,'<?php echo $theme;?>','blog',<?php echo $search_type;?>,<?php echo get_option('lightbox')?>, '<?php echo get_option('enable_message'); ?>', '<?php echo $noImage; ?>', '<?php echo $staff_uniqid;?>');
    </script>
</div>
