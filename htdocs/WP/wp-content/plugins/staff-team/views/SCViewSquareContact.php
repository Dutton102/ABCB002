...
foreach ($cont_posts as $key => $item) {
  ...
  /*content*/
  if(isset($params[0]['square_social_icons'])  && in_array(0,$params[0]['square_social_icons'])) {
    $cont['social_facebook']= get_post_meta($item->ID, 'social_facebook', TRUE);
  }else{
    $cont['social_facebook']="";
  }
  if(isset($params[0]['square_social_icons'])  && in_array(1,$params[0]['square_social_icons'])) {
    $cont['social_instagram']= get_post_meta($item->ID, 'social_instagram', TRUE);
  }else{
    $cont['social_instagram']="";
  }
  if(isset($params[0]['square_social_icons'])  && in_array(2,$params[0]['square_social_icons'])) {
    $cont['social_twitter']= get_post_meta($item->ID, 'social_twitter', TRUE);
  }else{
    $cont['social_twitter']="";
  }
  if(isset($params[0]['square_social_icons'])  && in_array(3,$params[0]['square_social_icons'])) {
  ...
  /*popup*/
  if(isset($params[0]['square_popup_social_icons'])  && in_array(0,$params[0]['square_popup_social_icons'])) {
    $cont['popup_social_facebook']= get_post_meta($item->ID, 'social_facebook', TRUE);
  }else{
    $cont['popup_social_facebook']="";
  }
  if(isset($params[0]['square_popup_social_icons'])  && in_array(1,$params[0]['square_popup_social_icons'])) {
    $cont['popup_social_instagram']= get_post_meta($item->ID, 'social_instagram', TRUE);
  }else{
    $cont['popup_social_instagram']="";
  }
  if(isset($params[0]['square_popup_social_icons'])  && in_array(2,$params[0]['square_popup_social_icons'])) {
    $cont['popup_social_twitter']= get_post_meta($item->ID, 'social_twitter', TRUE);
  }else{
    $cont['popup_social_twitter']="";
  }
  if(isset($params[0]['square_popup_social_icons'])  && in_array(3,$params[0]['square_popup_social_icons'])) {
...
$Square_conts = json_encode($conts);
/*- Contact's count -*/
$viewCount  = isset($params[0]['square_cont_count_in_page'])?$params[0]['square_cont_count_in_page']:'6';
...
<div class="<?php echo $theme; ?> sc_container" id="theme">
    <!-- Search -->
	<div id="square_search" class="search_<?php echo $staff_uniqid;?>">
	...
	<!-- Contacts -->
	<div id="square_contact" class="parentDiv">
    ...
    <script type="text/javascript">
      var showCase6_cont = new contactView(<?php echo $Square_conts;?>,<?php echo $viewCount;?>,'<?php echo $theme;?>','square',<?php echo $search_type;?>,<?php echo get_option('lightbox')?>, '<?php echo get_option('enable_message'); ?>' , '', '<?php echo $staff_uniqid;?>');
    </script>
</div>
