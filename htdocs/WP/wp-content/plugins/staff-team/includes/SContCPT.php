...
class SContCPT {
    ...
    private function __construct() {
        ...
        add_action('restrict_manage_posts', array($this, 'contRestrictManage'));
        add_filter('parse_query', array($this, 'contCatFilter'));
    ...
    public function contRestrictManage() {
        global $typenow;
        ;
    ...
    public function contCatFilter($request) {
        ...
        if ('edit.php' == $pagenow) {
            ...
            foreach ($filters as $tax_slug) {
                ...
                $var = &$request->query_vars[$tax_slug];
                if (isset($var)) {
                    if (is_numeric($var))
    ...
    public function postCustomFields($post) {
        ...
        $social_facebook = get_post_meta($post_id, 'social_facebook', TRUE);
        $social_instagram = get_post_meta($post_id, 'social_instagram', TRUE);
        $social_twitter = get_post_meta($post_id, 'social_twitter', TRUE);
        $social_google = get_post_meta($post_id, 'social_google', TRUE);
    ...
    public function saveContPost($ID = false) {
        ...
        if (isset($_POST['post_type']) && $_POST['post_type'] == self::$post_type) {
            ...
            if(isset($_POST['social_facebook'])){
               update_post_meta($ID, 'social_facebook', $_POST['social_facebook']);
            }
            if(isset($_POST['social_instagram'])){
               update_post_meta($ID, 'social_instagram', $_POST['social_instagram']);
            }
            if(isset($_POST['social_twitter'])){
               update_post_meta($ID, 'social_twitter', $_POST['social_twitter']);
            }
            if(isset($_POST['social_google'])){
               update_post_meta($ID, 'social_google', $_POST['social_google']);
            }
		   ...
		   if (isset($_POST['theme_reset'])) {
            ...
            if (!file_exists($theme_url)) {
                self::createThemeCSS($ID, $theme, $theme_url);
            } else {
                unlink($theme_url);
                self::createThemeCSS($ID, $theme, $theme_url);
            }
    ...
    public function createThemeCSS($theme_id, $theme, $theme_url) {
      $theme_folder_url = SC_DIR . '/css/themesCSS';
      if(is_writable($theme_folder_url)) {
          $themeCss = fopen($theme_url, "w");
          $cssData  = '';
          $params       = get_post_meta($theme_id, 'params');
          $param_values = $params[0];
          ob_start();
          include SC_DIR . '/includes/Main_Styles.php';
          $main_styles = substr(ob_get_clean(), 7);
          $cssData .= $main_styles;
          ob_start();
          include SC_DIR . '/includes/Animation.php';
          $animation = substr(ob_get_clean(), 7);
          $cssData .= $animation;
          ob_start();
          include SC_DIR . '/includes/ShowCases/1.ShortContactCss.php';
          $short_content = substr(ob_get_clean(), 7);
          $cssData .= $short_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/2.FullContactCss.php';
          $full_content = substr(ob_get_clean(), 7);
          $cssData .= $full_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/3.TableContactCss.php';
          $table_content = substr(ob_get_clean(), 7);
          $cssData .= $table_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/4.ChessContactCss.php';
          $showCase2_content = substr(ob_get_clean(), 7);
          $cssData .= $showCase2_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/5.PortfolioContactCss.php';
          $showCase3_content = substr(ob_get_clean(), 7);
          $cssData .= $showCase3_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/6.BlogContactCss.php';
          $showCase4_content = substr(ob_get_clean(), 7);
          $cssData .= $showCase4_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/7.CircleContactCss.php';
          $showCase5_content = substr(ob_get_clean(), 7);
          $cssData .= $showCase5_content;
          ob_start();
          include SC_DIR . '/includes/ShowCases/8.SquareContactCss.php';
          $showCase6_content = substr(ob_get_clean(), 7);
          $cssData .= $showCase6_content;
          ob_start();
          include SC_DIR . '/includes/SingleContactCss.php';
          $single_content = substr(ob_get_clean(), 7);
          $cssData .= $single_content;
          fwrite($themeCss, $cssData);
          fclose($themeCss);
