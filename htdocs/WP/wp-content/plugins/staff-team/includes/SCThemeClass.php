...
class SCThemeClass {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_activate_theme', array($this,'activateTheme'));
        ...
        add_action( 'wp_trash_post', array($this,'deleteTheme') );
    }
    ...
    public function activateTheme() {
        update_option('cont_active_theme', $_POST['id']);
        echo $_POST['id'];
        wp_die();
    }
    
    public function deleteTheme($post_id) {
        global $post_type;   
        if ( $post_type == self::$post_type ){
            ...
            if($active_theme == $post_id){
                ...
                if($postslist[0]->ID != $post_id){
                    update_option('cont_active_theme', $postslist[0]->ID);
                }else {
                    if(isset($postslist[1])){
                        update_option('cont_active_theme', $postslist[1]->ID);
                    }
                }
                return;
            }
